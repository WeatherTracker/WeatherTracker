        ...
        FirebaseMessaging.getInstance().getToken()
                .addOnCompleteListener(new OnCompleteListener<String>() {
                    ...
                    public void onComplete(@NonNull Task<String> task) {
                        ...
                        Log.d("FCM_token", task.getResult(), task.getException());
                ...
                if (android.util.Patterns.EMAIL_ADDRESS.matcher(email_text).matches()) {
                    if (password1.length() >= 8) {
                        if (password1.equals(password2)) {
                            ...
                            Call<Ack> call = retrofitService.send_registeredMail(email_text, password1, FCM_token);
                            call.enqueue(new Callback<Ack>() {
                                ...
                                public void onResponse(Call<Ack> call, Response<Ack> response) {
                                    if (!response.isSuccessful()) {
                                        ...
                                        if (ack.getCode() == 200) {
                                            Toast.makeText(SignUp.this, ack.getMsg(), Toast.LENGTH_SHORT).show();//去信箱收信
                                        } else {
                        ...
                        Toast.makeText(SignUp.this, "密碼長度須至少8個字元", Toast.LENGTH_SHORT).show();
                    }
                } else {
